  ...
  return (
    <header className="fixed top-6 md:top-4  left-1/2 w-[calc(100%-2rem)] md:w-auto  -translate-x-1/2 z-50 rounded-3xl bg-primary/80 px-5 py-2 backdrop-blur-md md:px-8">
      <div className="flex h-9 items-center justify-between">
          ...
          {navLinks.map((link) => (
              <NavLink key={link.path} to={link.path} className={({ isActive }) => navLinkClass(isActive, "p-2")} >
                {link.label}
              </NavLink>
          ))}
            ...
            {isSearchOpen && (
              <div className="absolute left-0 top-full mt-3">
                <input
                  type="text"
                  placeholder="جستجو..."
                  className={`${searchInputBaseClass} w-56 shadow-lg`}
                />
              </div>
            )}
          </div>
          ...
          <div className="border-t border-white/20 mt-3 py-2 md:hidden flex flex-col gap-2">
            {dropdownLinks.map((dropdown) => (
                <div key={dropdown.label}>
                  <Button onClick={() =>setOpenDropdown( openDropdown === dropdown.label ? null : dropdown.label)}
                  ...
                  {openDropdown === dropdown.label && (
                    <div className="mr-4 border-r border-white/20 pr-2">
                      {dropdown.items.map((item) => (
                        <Link key={item.path} to={item.path} onClick={() => { setIsMenuOpen(false); setOpenDropdown(null);}}
              ...
              {navLinks.map((link) => (
                <NavLink key={link.path} to={link.path} onClick={() => setIsMenuOpen(false)}
                  className={({ isActive }) => navLinkClass(isActive, "px-4 py-3 text-white")}>
                  {link.label}
                </NavLink>
              ))}
              <div className="mt-2 flex items-center gap-2">
                ...
                <input
                  type="text"
                  placeholder="جستجو..."
                  className={`${searchInputBaseClass} w-full py-2`}
                />
              </div>
          </div>
        )}
